import { useState } from 'react';
import { useTranslation } from 'react-i18next';
import TeamFlag from './TeamFlag';
import { formatMatchDate } from '../utils/matchTime';
import styles from './ExactScoreSpotlight.module.css';
import layout from './spotlightLayout.module.css';

function SpotlightBody({ entry, isYou, t, compact }) {
  const { match, prediction, name, points } = entry;
  const home = match.homeTeam;
  const away = match.awayTeam;
  const actualHome = match.score.fullTime.home;
  const actualAway = match.score.fullTime.away;

  return (
    <div className={`${styles.stage} ${isYou ? styles.stageYou : ''}`}>
      <div className={`${styles.winnerHero} ${compact ? layout.compactWinnerHero : ''}`}>
        <span className={styles.winnerLabel}>{t('spotlight.winner')}</span>
        <div className={styles.nameRow}>
          <span className={styles.nameFlank} aria-hidden="true" />
          <h3 className={`${styles.winnerName} ${compact ? layout.compactWinnerName : ''}`} title={name}>{name}</h3>
          <span className={styles.nameFlank} aria-hidden="true" />
        </div>
        {isYou && <span className={styles.youTag}>{t('spotlight.thatsYou')}</span>}
      </div>

      <div className={styles.scoreboard}>
        <div className={styles.scoreboardRing} aria-hidden="true" />
        <div className={`${styles.scoreboardInner} ${layout.scoreboardZone}`}>
          <div className={styles.teamSide}>
            <TeamFlag crest={home.crest} tla={home.tla} name={home.name} size={40} />
            <span className={styles.teamName}>{home.shortName || home.name}</span>
          </div>

          <div className={styles.scoreCenter}>
            <div className={styles.scoreline} aria-label={`${actualHome} – ${actualAway}`}>
              <span className={`${styles.scoreDigit} ${compact ? layout.compactScoreDigit : ''} numerals`}>{actualHome}</span>
              <span className={styles.scoreDash} aria-hidden="true">–</span>
              <span className={`${styles.scoreDigit} ${compact ? layout.compactScoreDigit : ''} numerals`}>{actualAway}</span>
            </div>
            <span className={styles.pointsBadge}>
              <span className={`${styles.pointsValue} numerals`}>+{points}</span>
              <span className={styles.pointsUnit}>{t('home.hero.pts')}</span>
              <span className={styles.exactDot} aria-hidden="true">·</span>
              <span className={styles.exactTag}>{t('spotlight.exactHit')}</span>
            </span>
            <p className={styles.pickLine}>
              {t('spotlight.predicted', {
                home: prediction.home,
                away: prediction.away,
              })}
            </p>
          </div>

          <div className={styles.teamSide}>
            <TeamFlag crest={away.crest} tla={away.tla} name={away.name} size={40} />
            <span className={styles.teamName}>{away.shortName || away.name}</span>
          </div>
        </div>
      </div>
    </div>
  );
}

function HistoryRow({ entry, locale }) {
  const home = entry.match.homeTeam;
  const away = entry.match.awayTeam;
  const actual = entry.match.score.fullTime;

  return (
    <li className={`${layout.historyRow} ${styles.historyRow}`}>
      <span className={`${styles.historyDate} numerals`}>
        {formatMatchDate(`${entry.date}T12:00:00.000Z`, locale, { day: 'numeric', month: 'short' })}
      </span>
      <span className={styles.historyName} title={entry.name}>{entry.name}</span>
      <span className={styles.historyTeams}>
        {home.tla ?? home.shortName} {actual.home}–{actual.away} {away.tla ?? away.shortName}
      </span>
      <span className={`${styles.historyPts} numerals`}>+{entry.points}</span>
    </li>
  );
}

export default function ExactScoreSpotlight({ data, loading, currentUserId, compact }) {
  const { t, i18n } = useTranslation();
  const locale = i18n.resolvedLanguage === 'he' ? 'he-IL' : 'en-GB';
  const [historyOpen, setHistoryOpen] = useState(false);

  if (loading) {
    return (
      <article className={`${styles.card} ${layout.cardShell} ${styles.cardSkeleton}`} aria-busy="true" aria-label={t('spotlight.winner')}>
        <div className={styles.skelName} />
        <div className={styles.skelScore} />
      </article>
    );
  }

  if (!data?.primary) return null;

  const { primary, history } = data;
  const isYou = primary.userId === currentUserId;

  const contextLine = (() => {
    if (primary.soloExact && primary.exactCountOnDay === 1) {
      return t('spotlight.contextSolo');
    }
    if (primary.soloExact) {
      return t('spotlight.contextSoloAmong', { count: primary.exactCountOnDay });
    }
    return t('spotlight.contextShared', {
      count: primary.exactCountOnDay,
      matchCount: primary.coExactOnMatch,
    });
  })();

  return (
    <article className={`${styles.card} ${layout.cardShell} ${isYou ? styles.cardYou : ''}`} aria-label={t('spotlight.winner')}>
      <div className={styles.glow} aria-hidden="true" />
      <div className={styles.beam} aria-hidden="true" />

      <div className={layout.cardMain}>
        <SpotlightBody entry={primary} isYou={isYou} t={t} compact={compact} />
      </div>

      <div className={layout.cardFooter}>
        <p className={`${styles.context} ${layout.contextBlock}`}>{contextLine}</p>

        {history.length > 0 && (
          <div className={layout.historyBlock}>
            <button
              type="button"
              className={`${layout.historyToggle} ${styles.historyToggle}`}
              onClick={() => setHistoryOpen((v) => !v)}
              aria-expanded={historyOpen}
            >
              <span>{t('spotlight.pastWinners', { count: history.length })}</span>
              <span className={`${styles.chevron} ${historyOpen ? styles.chevronOpen : ''}`} aria-hidden="true">
                ▾
              </span>
            </button>
            {historyOpen && (
              <ol className={layout.historyList}>
                {history.map((entry) => (
                  <HistoryRow key={entry.date} entry={entry} locale={locale} />
                ))}
              </ol>
            )}
          </div>
        )}
      </div>
    </article>
  );
}
