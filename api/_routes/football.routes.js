// /api/football/* — thin, typed proxy to TheSportsDB.
// Frontend never sees TheSportsDB field names; everything is normalized
...
function sendUpstreamError(res, err, next) {
  if (err.response) {
    ...
    if (status === 403) return res.status(403).json({ error: 'Football API key is invalid.' });
